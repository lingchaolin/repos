...
//	 * 分析请求信息
	private void parseRequestInfo(){
		...
		/**
		 * 从信息的首行分解出 :请求方式 (post/get)   请求路径   请求参数(get可能存在)
		...
		this.method=firstLine.substring(0, idx).trim();//取到第一个空格前的字符串
		String urlStr =firstLine.substring(idx,firstLine.indexOf("HTTP/")).trim();
		if(this.method.equalsIgnoreCase("post")){//post方式下：
			this.url=urlStr;		
