...
<script type="text/javascript" src="<%=path%>/card/script/js/jquery-1.8.1.min.js"></script>
...
<script type="text/javascript">
	function init(){
		...
		$.post("<%=path%>/spring/station",params,function(data){
	...
	function showInfo(thisMarker, point) {
		...
		var sContent = '<ul style="margin:0 0 5px 0;padding:0.2em 0">'
				...
				+ point.stationName
				+ '</li>'
				+'<li style="line-height: 26px;font-size: 15px;"></span><a href="<%=path%>/card/turnCard.jsp">办卡</a></br><a href="<%=path%>/card/turnMain.jsp">充值</a></br><a href="<%=path%>/card/turnOut.jsp">车辆调出</a></br><a href="<%=path%>/card/turnIn.jsp">车辆调入</a></br><a href="<%=path%>/card/turnSelect.jsp">卡费用统计</a></li>'  
			    +'</ul>';
