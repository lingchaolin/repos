...
<script language="javascript" type="text/javascript">
	function getMoneyFirst(){
		//document.getElementById("getPayType").value="货到付款";
		document.getElementById("getPayType").value="110";
		document.getElementById("getPayTypeForm").submit();
	}
	function getPreMoneyFirst(){
		//document.getElementById("getPayType").value="预付款到发货";
		document.getElementById("getPayType").value="130";
		document.getElementById("getPayTypeForm").submit();
	}
	function giveGoodFirst(){
		//document.getElementById("getPayType").value="款到发货";
		document.getElementById("getPayType").value="120";
		document.getElementById("getPayTypeForm").submit();
	}
	function inStock(obj){
		if(window.confirm("确定要将该采购单入库吗？")){
			document.getElementById("getInStock").value=obj.parentNode.parentNode.cells[0].children[0].innerHTML;
			document.getElementById("getInStockForm").submit();
		}
	}
	function ajax_getPoitem(obj){
		...
		httpRequest.open("GET","PurchaseShowInStockSub?poid="+poid,true);
...
<!--     <td width="30px" nowrap class="toolbar">&nbsp;</td>
	...
	<form action="PurchaseShowInStockMain" id="getPayTypeForm"><!--付款方式-->
		<input type="hidden" id="getPayType" name="getPayType"/>
	</form>
	<form action="PurchaseChangeInStockServ" id="getInStockForm"><!--进行入库操作的的单号-->
		<input type="hidden" id="getInStock" name="getInStock"/>
	</form>
...
<c:forEach items="${requestScope.pmsmList}" var="per" varStatus="status">
<tr>
    ...
    <td align="center"><a onclick="inStock(this)">入库</a></td>
  </tr>
  </c:forEach>
</table>
<br/>
